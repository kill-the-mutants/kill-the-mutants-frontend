...
var getExampleContents = function(access_token, repo, callback) {
  var query = {
    access_token : access_token,
  ...
  var options = {
    host : 'api.github.com',
    method: 'GET',
    path : '/repos/' + repo.owner + '/' + repo.repo + '/git/trees/master?recursive=1?' + querystring.stringify(query),
    headers : headers
  };
  makeRequest(options, function(response) {
    // if the repo couldn't be found
    if (response.message === 'Not Found') {
      // try forking https://github.com/kill-the-mutants/kill-the-mutants
      if (fork_attempts < 2) {
        console.log("Repo not found. Fork attempt #" + (fork_attempts + 1));
        fork_repo = {
          owner: 'kill-the-mutants',
          repo: 'kill-the-mutants'
        };
        fork(access_token, fork_repo, function(data) {
          fork_attempts++;
          getExampleContents(access_token, repo, callback);
          return;
        });
      }
      else {
        fork_attempts = 0;
        callback("There was a problem forking the kill-the-mutants repository.");
      }
      return;
    } else {
...
var getFileContents = function(access_token, repo, callback) {
  var query = {
    access_token : access_token,
    client_id : api.GITHUB_CLIENT_ID,
    client_secret : api.GITHUB_CLIENT_SECRET
  ...
  var options = {
    host : 'api.github.com',
    method: 'GET',
    path : '/repos/' + repo.owner + '/' + repo.repo + '/contents/' + repo.path + '?' + querystring.stringify(query),
    headers : headers
  };
  makeRequest(options, function(response) {
    if(callback) {
...
module.exports = {
  ...
  getUser: getUser,
  fork: fork,
  getExampleContents: getExampleContents,

...
function create_arguments(user, testname, git_push, test_tool, callback) {
  ...
  docker_arguments.tag = user.login + '-' + testname + '-' + docker_arguments.timestamp;
  docker_arguments.tag = docker_arguments.tag.toLowerCase(); // docker containers need to be lowercase

  // write to file arguments
  docker_arguments.filepath = path.join(__dirname, '../docker/tmp/' + docker_arguments.tag + '/Tests.java');

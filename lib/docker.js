...
function execute(user, testname, git_push, test_tool, code, callback) {
  create_arguments(user, testname, git_push, test_tool, function(docker_arguments) {
...
function create_arguments(user, testname, git_push, test_tool, callback) {
  ...
  var tag = user.login + '-' + testname + '-' + timestamp;

  var docker_arguments = {};

  // write to file arguments
  docker_arguments.filepath = path.join(__dirname, '../docker/tmp/' + user.login + '/' + testname + '/' + timestamp + '/Tests.java');

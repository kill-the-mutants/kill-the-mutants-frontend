...
router.post('/run-tests', function(req, res) {
  ...
  docker.execute(user, testname, false, 'junit', code, function(stdout, stderr) {
  ...
  docker.execute(user, testname, false, 'pit', code, function(docker_arguments, stdout, stderr) {
    ...
    results.store_results(user, testname, code, docker_arguments, stdout, stderr, function(db_output, err){
      ...
      if (compileErrs !== null) {
        ...
        stderr = "Your test suite is not currently passing! Please click \"Run Tests\" in order to see information on the failing test(s).";
      } else {
        stderr = null;
      }
